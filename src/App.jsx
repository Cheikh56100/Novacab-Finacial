...
const STORAGE="nfi-companies-v4";

export default function App(){
 const params=new URLSearchParams(window.location.search); const clientParam=params.get("client"); const sirenParam=params.get("siren");
 const [page,setPage]=useState(clientParam?"analysis":"dashboard"); const [authUser,setAuthUser]=useState(null); const [authReady,setAuthReady]=useState(!supabaseReady); const [syncError,setSyncError]=useState("");
 const [companies,setCompanies]=useState(()=>{try{const x=JSON.parse(localStorage.getItem(STORAGE));return Array.isArray(x)&&x.length?x:demoCompanies}catch{return demoCompanies}});
 ...
 useEffect(()=>{let active=true;if(!supabase){setCompany(companies.find(c=>String(c.novacabClientId||"")===String(clientParam)||(sirenParam&&String(c.siren||"")===String(sirenParam)))||companies[0]||null);setAuthReady(true);return;}
    ...
    if(user){
      try{
        ...
        if(!novacabUser?.portefeuille_id){
          setAuthUser(null);
          setSyncError("Votre compte Supabase n'est pas rattaché à un portefeuille NOVACAB actif. NFI ne peut pas déterminer le cabinet à afficher.");
        }else{
          setAuthUser(user);
   ...
   const {data:{subscription}}=supabase.auth.onAuthStateChange(async(_event,session)=>{
     ...
     if(session?.user){
       try{
         const novacabUser=await getCurrentNovacabUser();
         if(!novacabUser?.portefeuille_id) throw new Error("Votre compte n'est pas rattaché à un portefeuille NOVACAB actif.");
         const state=await loadNfiState();
         if(state){
           setCompanies(state.companies);
           const found=state.companies.find(c=>String(c.novacabClientId||"")===String(clientParam)||(sirenParam&&String(c.siren||"")===String(sirenParam)))||state.companies[0]||null;
 ...
 useEffect(()=>{if(!supabaseReady||!authUser)localStorage.setItem(STORAGE,JSON.stringify(companies))},[companies,authUser]);
 ...
 const removeCompany=async c=>{if(!c)return;const ok=window.confirm(`Supprimer « ${c.name} » et les exercices FEC associés de NFI ? Cette action est irréversible.`);if(!ok)return;try{await deleteCompany(c.id);setCompanies(prev=>prev.filter(x=>x.id!==c.id));if(company?.id===c.id){setCompany(null);setPage("companies");}}catch(e){setSyncError(e.message)}};
 ...
 return <div className="app"><Sidebar page={page} setPage={setPage}/><div className="main">{syncError&&<div className="notice warningNotice topNotice">Synchronisation : {syncError}</div>}<Topbar onSignOut={async()=>{try{if(supabase) await supabase.auth.signOut();setAuthUser(null);setCompany(null);setPage("dashboard")}catch(e){setSyncError(e.message);throw e}} } page={page} companies={companies} onOpenCompany={openCompany} setPage={setPage} authUser={authUser}/>
   {page==="dashboard"&&<Home companies={companies} setPage={setPage} onOpenCompany={openCompany}/>} 
