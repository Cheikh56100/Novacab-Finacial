...
export default function Companies({companies=[],setCompany,onAdd,onImportCompany,onDeleteCompany}){
 ...
 return <main className="content">
   <header className="pageHero"><div><div className="eyebrow">PORTEFEUILLE CABINET</div><h1>Portefeuille du cabinet</h1><p>Tous les dossiers du cabinet sont accessibles dans NOVACAB Insight pour permettre les analyses et comparaisons transversales.</p></div><button className="secondaryAction" onClick={onAdd}><UploadCloud size={15}/> Importer un FEC</button></header>
   <div className="toolbar"><div className="searchBox large"><Search size={16}/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Rechercher une société, un secteur, un SIREN..."/></div><span className="toolbarHint"><Database size={13}/> {companies.length} dossier{companies.length>1?"s":""}</span></div>
