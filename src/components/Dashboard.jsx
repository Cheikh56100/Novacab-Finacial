...
import {ratios,yoy,detailedAnalysis,financialScore,scoreLabel,scoreBreakdown,ebeWaterfall} from "../services/financialEngine";
import AnalysisDeepDive from "./AnalysisDeepDive";
import CopilotDiagnostic from "./CopilotDiagnostic";
...
export default function Dashboard({company,year,setYear,setPage,allCompanies=[]}){
 ...
 return <main className="content">
  <header className="pageHero analysisHero"><div><div className="eyebrow">ANALYSE FINANCIÈRE · {currentYear}</div><h1>{company.name}</h1><p>{company.naf||"NAF à renseigner"} · {company.sector||"Secteur à classer"}</p></div><div className="heroActions"><><button className="secondaryAction" onClick={()=>setPage("benchmark")}>Comparer au secteur</button><button className="primaryAction" onClick={()=>setPage("report")}>Générer le rapport client</button></><label className="yearSelect"><span>Exercice</span><select value={currentYear} onChange={e=>setYear(Number(e.target.value))}>{years.map(x=><option key={x}>{x}</option>)}</select></label></div></header>
  <section className="diagnosisHero"><div><div className="eyebrow">LECTURE NOVACAB INSIGHT</div><h2>{r.roe>=10&&r.gearing<=1.5?"Situation financière favorable":r.roe<0||r.gearing>3?"Points de vigilance identifiés":"Situation financière à surveiller"}</h2><p>{global}</p></div><button className="ghostAction" onClick={()=>setPage("benchmark")}>Voir le positionnement sectoriel <TrendingUp size={15}/></button></section>
  <CopilotDiagnostic company={company} year={currentYear} setPage={setPage} allCompanies={allCompanies}/>
  ...
  <section className="dashboardScoreGrid"><section className="panel scoreExplainPanel"><div className="scoreExplainHead"><div><div className="eyebrow">NOVACAB INSIGHT SCORE</div><h2>{financialScore(r)}/100 <span>{scoreLabel(financialScore(r))}</span></h2></div><button className="secondaryAction" onClick={()=>setPage("report")}>Voir le rapport</button></div><div className="scoreBars">{scoreBreakdown(r).map(x=><div key={x.key}><div><span>{x.label}</span><b>{x.score}/100 · {x.weight}%</b></div><div className="scoreBar"><i style={{width:`${x.score}%`}}/></div></div>)}</div></section><section className="panel dataQualityPanel"><div className="panelHead"><div><h2>Qualité des données</h2><p>Ce qui peut être considéré comme fiable avant diffusion.</p></div></div><div className={`qualityState ${company.quality?.balanceBalanced&&company.quality?.rowCount?"ok":"warning"}`}><strong>{company.quality?.rowCount?`${company.quality.rowCount.toLocaleString("fr-FR")} lignes FEC`:`FEC indisponible`}</strong><span>{company.quality?.exerciseCount||years.length} exercice(s) · équilibre {company.quality?.balanceBalanced?"OK":"à contrôler"}</span></div><ul className="qualityList"><li>{company.quality?.rowCount?"FEC chargé et exploitable":"Aucune écriture FEC exploitable"}</li><li>{company.quality?.balanceBalanced?"Débit / crédit équilibrés":"Écart débit / crédit à rapprocher"}</li><li>{company.validation?.warnings?.length?`${company.validation.warnings.length} avertissement(s) à revoir`:"Aucun avertissement de validation majeur"}</li></ul></section></section>
  <section className="panel"><div className="panelHead"><div><h2>Rentabilité & structure financière</h2><p>Les indicateurs demandés, calculés sur l'exercice sélectionné.</p></div></div><div className="ratioTable richRatioTable">{[["Gearing",fmtX(r.gearing)],["ROE",fmt(r.roe)],["ROCE",fmt(r.roce)],["Liquidité",fmt(r.liquidity)],["TMG",fmt(r.tmg)]].map(([n,v])=><Metric key={n} name={n} value={v}/>)}</div></section>
  ...
  <section className="panel waterfallPanel"><div className="panelHead"><div><h2>Waterfall EBE</h2><p>Pourquoi la rentabilité a changé entre N-1 et N. Le résiduel est explicitement isolé pour éviter de présenter une cause non documentée comme certaine.</p></div></div>{(()=>{const wf=ebeWaterfall(company,currentYear); if(!wf.available)return <div className="emptyState compact">Pas d'exercice N-1 disponible : le waterfall causal sera proposé dès qu'une comparaison historique est possible.</div>; const max=Math.max(...wf.items.map(x=>Math.abs(Number(x.value)||0)),1); return <div className="waterfallGrid">{wf.items.map((x,i)=><div className={`waterfallStep ${x.type}`} key={x.key}><span>{x.label}</span><b>{eur(x.value)}</b><i style={{width:`${Math.min(100,Math.max(8,Math.abs(Number(x.value)||0)/max*100))}%`}}/></div>)}</div>})()}</section>
  <AnalysisDeepDive company={company} year={currentYear}/>
