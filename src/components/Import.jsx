...
import {buildNovacabFinancialKpis} from "../services/novacabFinancial";
...
export default function Import({onImported,pendingCompany,companies=[],standalone=false}){
 ...
 const handle=async e=>{const f=e.target.files?.[0];if(!f)return;try{
   ...
   const novacabFinancialKpis=buildNovacabFinancialKpis(rows,f.name);
   ...
   c.novacabFinancialKpis=novacabFinancialKpis;
