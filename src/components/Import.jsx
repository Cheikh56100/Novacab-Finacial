...
export default function Import({onImported,pendingCompany,companies=[]}){
 ...
 const handle=async e=>{const f=e.target.files?.[0];if(!f)return;try{
   if(!selectedCompany) throw new Error("Sélectionnez d'abord un dossier NOVACAB.");
   ...
   const c={...detected,id:selectedCompany.id,novacabClientId:selectedCompany.id,name:selectedCompany.name,siren:expectedSiren||fecSiren,naf:selectedCompany.naf||detected.naf||"",sector:selectedCompany.sector&&selectedCompany.sector!=="À classer"?selectedCompany.sector:(detected.sector||"À classer")};
 ...
 const save=()=>{if(!company||!selectedCompany)return;const c={...company,id:selectedCompany.id,novacabClientId:selectedCompany.id,name:selectedCompany.name,siren:selectedCompany.siren,naf:selectedCompany.naf,sector:selectedCompany.sector||"À classer",pending:false};onImported(c);setCompany(c);setStatus({type:"success",text:"FEC enregistré dans le dossier NOVACAB sélectionné."});};
 const sectorOptions=[...new Set(nafCatalog.map(x=>x["Secteur d'activité"]).filter(Boolean))].sort();
 return <main className="content"><header className="pageHero"><div><div className="eyebrow">DONNÉES · IMPORT FEC</div><h1>Importer un FEC</h1><p>Le FEC est rattaché à un dossier existant de votre portefeuille NOVACAB.</p></div></header>
