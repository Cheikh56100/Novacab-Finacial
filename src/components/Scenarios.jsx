...
export default function Scenarios({company}){
 ...
 return <main className="content"><header className="pageHero"><div><div className="eyebrow">PRÉVISIONNEL NOVACAB INSIGHT</div><h1>Prévisionnel recommandé</h1><p>{company.name} · proposition construite à partir de la situation financière</p></div></header>
 <section className="forecastHero panel"><div className="forecastIcon"><Sparkles size={22}/></div><div><span className="eyebrow">RECOMMANDATION NOVACAB INSIGHT</span><h2>{rec.profile}</h2><p>{rec.reason}</p></div><div className="forecastScore"><b>{rec.score}</b><span>/100 · {rec.label}</span></div></section>
 ...
 <div className="panel"><div className="panelHead"><div><h2>Hypothèses recommandées</h2><p>Tu peux les modifier avant de valider</p></div><button className="secondaryAction" onClick={apply}><Target size={14}/> Appliquer NOVACAB Insight</button></div>{[["ca","Variation CA (%)"],["margin","Variation marge (points)"],["bfr","Variation BFR (%)"],["investments","Investissements (€)"],["debtRepayment","Remboursement dette (€)"]].map(([k,l])=><label className="forecastField" key={k}>{l}<input type="number" value={a[k]} onChange={e=>set(k,e.target.value)}/></label>)}</div></section>
