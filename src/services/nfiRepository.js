...
export async function getNfiSession() {
...
export async function getCurrentNovacabUser() {
  ...
  const { data, error } = await supabase.from("team")
    ...
    .maybeSingle();
  if (error) throw error;
  if (!data) return { id:user.id, auth_user_id:user.id, nom:user.user_metadata?.full_name || user.email || "Utilisateur", email:user.email || "", role:"collaborateur" };
  return data;
}
...
export async function deleteCompany(clientId) {
  ...
  await supabase.from("nfi_financial_analyses").delete().eq("client_id",String(clientId));
  await supabase.from("nfi_forecasts").delete().eq("client_id",String(clientId));
}
