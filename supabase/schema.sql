...
-- ------------------------------------------------------------
-- Helpers de sécurité : un utilisateur peut appartenir à plusieurs cabinets.
-- Insight construit son périmètre à partir de TOUTES ses lignes actives dans team.
-- NOVACAB reste la source de vérité des organisations et des clients.

create or replace function public.nfi_current_team_id()
returns text
language sql stable security definer set search_path=public
as $$
  select t.id::text from public.team t
  where t.auth_user_id = auth.uid() and coalesce(t.statut,'actif') = 'actif'
  order by t.cabinet_nom nulls last, t.id
  limit 1;
$$;
...
language sql stable security definer set search_path=public
as $$
  select coalesce((select t.role from public.team t where t.auth_user_id = auth.uid() and coalesce(t.statut,'actif')='actif' order by t.cabinet_nom nulls last, t.id limit 1),'collaborateur');
$$;
...
language sql stable security definer set search_path=public
as $$
  select distinct on (t.portefeuille_id::text)
    t.portefeuille_id::text,
    coalesce(nullif(t.cabinet_nom,''),'Cabinet '||t.portefeuille_id::text),
    t.role,
    t.id::text
  from public.team t
  where t.auth_user_id = auth.uid()
    and coalesce(t.statut,'actif')='actif'
    and t.portefeuille_id is not null
...
as $$
  ...
  where c.portefeuille_id::text in (
    select t.portefeuille_id::text from public.team t
    where t.auth_user_id=auth.uid() and coalesce(t.statut,'actif')='actif' and t.portefeuille_id is not null
  )
...
as $$
  select c.id::text, c.data, c.portefeuille_id::text
  from public.clients c where c.id::text=p_client_id and public.nfi_can_access_client(c.id::text);
$$;
...
as $$
  ...
  where coalesce(t.statut,'actif')='actif'
    and t.portefeuille_id::text in (select x.portefeuille_id::text from public.team x where x.auth_user_id=auth.uid() and coalesce(x.statut,'actif')='actif' and x.portefeuille_id is not null)
